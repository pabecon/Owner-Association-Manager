...
export default function PermissionsMatrix() {
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto" data-testid="page-permissions-matrix">
      <div className="flex items-center gap-3 flex-wrap">
        <Shield className="w-6 h-6 text-primary" />
        <h1 className="text-xl font-bold" data-testid="text-permissions-title">Matrice Permisiuni</h1>
      </div>
            ...
            <table className="w-full text-sm" data-testid="table-permissions-matrix">
              <thead>
                <tr className="border-b">
                  <th className="text-left p-3 font-medium text-muted-foreground min-w-[250px]">Actiune / Raport</th>
                  {roles.map((role) => (
                    <th key={role} className="text-center p-3 font-medium min-w-[100px]" data-testid={`th-role-${role}`}>
                ...
                return (
                  <tbody key={category}>
                    <tr className="bg-muted/50">
                      <td colSpan={roles.length + 1} className="p-2 px-3">
                    ...
                    {entries.map((entry) => (
                      <tr key={entry.key} className="border-b last:border-b-0" data-testid={`row-perm-${entry.key}`}>
                        <td className="p-2.5 px-3 text-sm">{entry.label}</td>
                        {roles.map((role) => (
