...
export default function Explorer() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold tracking-tight" data-testid="text-explorer-title">Explorator Ierarhie</h1>
        <p className="text-muted-foreground text-sm mt-1">Navigheaza prin structura completa a administrarii</p>
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3].map(i => (
            <Card key={i}><CardContent className="p-5 space-y-3"><Skeleton className="h-6 w-32" /><Skeleton className="h-4 w-full" /></CardContent></Card>
          ))}
        </div>
      ) : (
        <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {floors.length === 0 && (
                  <Card><CardContent className="flex flex-col items-center justify-center py-12">
                    ...
                    <p className="text-muted-foreground text-sm">Nicio planta cu unitati in aceasta scara</p>
                  </CardContent></Card>
                )}
              </div>
            </div>
          ...
          {currentLevel === "floor" && selectedStaircase && selectedFloor !== null && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {floorUnits.map(unit => {
                ...
                const UnitIcon = UNIT_TYPE_ICONS[uType] || Home;
                const typeLabel = UNIT_TYPE_LABELS[uType] || "Apartament";
                return (
                  <Card key={unit.id} className="hover-elevate" data-testid={`explorer-unit-${unit.id}`}>
                    <CardContent className="p-5">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <div className="flex items-center justify-center w-8 h-8 rounded-md bg-primary/10">
                            <UnitIcon className="w-4 h-4 text-primary" />
                          </div>
                          <span className="text-lg font-bold">{typeLabel} {unit.number}</span>
                        </div>
                      ...
                      <div className="space-y-1.5">
                        {unit.ownerName && (
                          <div className="flex items-center gap-2 text-sm">
                            <User className="w-3.5 h-3.5 text-muted-foreground" /><span>{unit.ownerName}</span>
                          </div>
                        )}
                        {unit.ownerPhone && (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Phone className="w-3.5 h-3.5" /><span>{unit.ownerPhone}</span>
                          </div>
                        )}
                        {unit.ownerEmail && (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            <Mail className="w-3.5 h-3.5" /><span className="truncate">{unit.ownerEmail}</span>
                          </div>
                        )}
                      </div>
                      <div className="flex items-center gap-3 mt-3 pt-3 border-t text-xs text-muted-foreground flex-wrap">
                        {unit.surface && <span>{unit.surface} mp</span>}
                        {unit.rooms && <span>{unit.rooms} camere</span>}
                        {unit.residents && <span>{unit.residents} {Number(unit.residents) === 1 ? "persoana" : "persoane"}</span>}
                      </div>
                    </CardContent>
                  </Card>
                );
