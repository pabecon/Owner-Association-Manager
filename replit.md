...
- **Contract Template Builder (`/sabloane`):** Structured template system with a chapter catalog (19 predefined types: Antet, Identificarea Părților, Preambul, etc.) and article builder. Templates have a tree structure: Template → Chapters → Articles. Articles support database field references (`{{association.name}}`, `{{contract.startDate}}`, etc.) that resolve to actual data when generating contracts. Chapters can be selected from the catalog or custom-named. The system also supports traditional file-upload templates (PDF, DOC). Database tables: `contract_chapter_catalog`, `template_chapters`, `template_articles`.
